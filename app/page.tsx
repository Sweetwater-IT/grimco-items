...
        break
      case "lastPurchased":
        // Date comparison (assuming a consistent format that Date.parse can handle, e.g., "YYYY-MM-DD" or "Month DD, YYYY")
        // If the date format is just "Mon YYYY", it's better to use a custom parsing logic if sorting needs to be perfect for generated data. 
        // For simplicity, using Date.parse here, but if the dates are "Jan 2024", "Feb 2024", etc., simple string sort will also fail.
        // It's safer to assume the date strings in the `PURCHASE_HISTORY` (if real) are sortable or can be parsed.
        comparison = Date.parse(aValue as string) - Date.parse(bValue as string)
        break
      default:
        // String comparison (for 'item' and 'description')
